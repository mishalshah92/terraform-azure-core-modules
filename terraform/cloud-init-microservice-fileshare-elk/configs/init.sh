...
# ELK
ELK_SETTINGS=${elk_settings}
ELK_DATA_PATH=${elk_data_path}
...

{
  ...
  then
    ...
    if [ -z "$(grep "$smbPath"\ "$FILE_SHARE_MOUNT_PATH" /etc/fstab)" ]; then
        echo "$smbPath $FILE_SHARE_MOUNT_PATH cifs nofail,uid=$FILE_SHARE_UID,gid=$FILE_SHARE_GID,vers=3.0,credentials=$smbCredentialFile,serverino" | sudo tee -a /etc/fstab > /dev/null
    else
  ...
  # ELK Settings
  if [[ "$ELK_SETTINGS" == "true" ]] && [[ "$ELK_SETTINGS" != "null" ]]
  then
    echo "INFO: Applying ELK settings."

    echo 'vm.max_map_count=262144' >> /etc/sysctl.conf
    sysctl -w vm.max_map_count=262144

    if [ ! -d "$ELK_DATA_PATH" ]; then
        mkdir -p "$ELK_DATA_PATH"
    fi

    chmod g+rwx "$ELK_DATA_PATH"
    chgrp 0 "$ELK_DATA_PATH"

    echo "INFO: Applying ELK settings is complete."
  fi
